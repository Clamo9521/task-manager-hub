// Home Task Manager JavaScript
console.log("Home Task Manager loaded!");
...
function saveTasks() {
    localStorage.setItem('homeTaskManagerTasks', JSON.stringify(tasks));
}

function loadTasks() {
    const savedTasks = localStorage.getItem('homeTaskManagerTasks');
    if (savedTasks) {
...
function updateStats() {
    ...
    statsText.textContent = `${emoji} ${totalTasks} home tasks • ${completedTasks} done • ${pendingTasks} pending • ${completionRate}% complete`;
}
...
function addTask() {
    ...
    if (taskText === '') {
        alert('Please enter a home task!');
        return;
    }
...
function clearAllTasks() {
    ...
    if (confirm('Are you sure you want to clear all home tasks? This cannot be undone.')) {
        tasks = [];
        saveTasks();
        renderTasks();
        updateStats();
