// Learning Task Manager JavaScript
console.log("Learning Task Manager loaded!");
...
function saveTasks() {
    localStorage.setItem('learningTaskManagerTasks', JSON.stringify(tasks));
}

function loadTasks() {
    const savedTasks = localStorage.getItem('learningTaskManagerTasks');
    if (savedTasks) {
...
function updateStats() {
    ...
    statsText.textContent = `${emoji} ${totalTasks} learning tasks • ${completedTasks} mastered • ${pendingTasks} pending • ${completionRate}% complete`;
}
...
function addTask() {
    ...
    if (taskText === '') {
        alert('Please enter a learning task!');
        return;
    }
...
function clearAllTasks() {
    ...
    if (confirm('Are you sure you want to clear all learning tasks? This cannot be undone.')) {
        tasks = [];
        saveTasks();
        renderTasks();
        updateStats();
