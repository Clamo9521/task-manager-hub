// Work Task Manager JavaScript
console.log("Work Task Manager loaded!");
...
function saveTasks() {
    localStorage.setItem('workTaskManagerTasks', JSON.stringify(tasks));
}

function loadTasks() {
    const savedTasks = localStorage.getItem('workTaskManagerTasks');
    if (savedTasks) {
...
function updateStats() {
    ...
    statsText.textContent = `${emoji} ${totalTasks} work tasks • ${completedTasks} done • ${pendingTasks} pending • ${completionRate}% complete`;
}
...
function addTask() {
    ...
    if (taskText === '') {
        alert('Please enter a work task!');
        return;
    }
...
function clearAllTasks() {
    ...
    if (confirm('Are you sure you want to clear all work tasks? This cannot be undone.')) {
        tasks = [];
        saveTasks();
        renderTasks();
        updateStats();
